...
def extract_mfcc(audio_file):
    ...
    try:
        ...
        print(f"{audio_file} başarıyla yüklendi.")
        duration = librosa.get_duration(y=y, sr=sr)
        print(f"{audio_file} uzunluğu: {duration:.2f} saniye")
        if duration < 1.0:
            print(f"{audio_file} çok kısa!")
        ...
        print(f"{audio_file} yüklenemedi: {e}")
        return None

def train_model():
    """ SVM modelini eğitir. """
    os.chdir("C:\\seslirobot")  # Çalışma dizinini ayarla
    ...
    print(f"Bulunan konuşmacı klasörleri: {speaker_folders}")

    for folder_name in speaker_folders:
        print(f"Klasör işleniyor: {folder_name}")
        speaker_name = folder_name[:-4]  # "_Ses" kısmını kaldır
    ...
    print(f"Toplam işlenen ses dosyası sayısı: {len(X)}")
    print(f"Toplam konuşmacı sayısı: {len(set(y))}")
...
class App(tk.Tk):
    ...
    def stop_recording(self):
        ...
        wf.close()
        messagebox.showinfo("Başarılı", "Ses kaydı tamamlandı!")
    ...
    def analyze_recording(self):
        if self.svm_model:
            try:
                self.progress['value'] = 0
                self.update()
                mfccs = extract_mfcc("kaydedilen_ses.wav")
                if mfccs is not None:
                    speaker_id = self.svm_model.predict([mfccs])[0]
                    self.speaker_label.config(text=f"Konuşmacı: {speaker_id}")
